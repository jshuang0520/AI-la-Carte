...
class FilterHelper:
    ...
    def get_recommendations(
        ...
        """
        Main recommendation function that coordinates RAG, geo filtering,
        and translations.
        """
        try:
    ...
    def _filter_stores(
        ...
        """
        try:
            requested_date = user_preferences.get(
            ...
            distance_dataframe.loc[:, 'Date_of_Last_SO'] = pd.to_datetime(
                distance_dataframe['Date_of_Last_SO'], unit='ms'
            ).dt.date
    ...
    def _mix_markets_SP_HOO(self) -> pd.DataFrame:
        """
        Given markets and shopping partner data, merge them into a single
        dataframe.
        """
        try:
            ...
            for i in range(5):
                ...
                HOO_dataframe.loc[
                    HOO_dataframe["Frequency"].str.contains(str(i + 1)),
                    f"Week {i + 1}"
                ] = True
    ...
    def _add_weeks_present(
        ...
        """
        Given date information, find 8 weeks from the start of the date's month
        when the shop/market is open.
        """
        ...
        HOO_dist_df["This Month's 1st day"] = datetime.datetime(
            requested_date.year, requested_date.month, 1
        )
        HOO_dist_df["This Month's 1st day"] = pd.to_datetime(
            HOO_dist_df["This Month's 1st day"]
        )
        HOO_dist_df["Next Month's 1st day"] = (
            HOO_dist_df["This Month's 1st day"] + pd.DateOffset(months=1)
        )
        ...
        for which_month in ["This", "Next"]:  # Running on month
            for i in range(5):  # Running on week
                ...
                HOO_dist_df[f"{which_month} Week {i + 1}'s date"] = (
                    HOO_dist_df[f"{which_month} Month's 1st day"] + time_delta
                )

                # Check whether the date is in the same month
                not_same_month = (
                    HOO_dist_df[f"{which_month} Week {i + 1}'s date"].dt.month
                ...
                HOO_dist_df.loc[
                    ~HOO_dist_df[f"Week {i + 1}"], f"{which_month} Week {i + 1}'s date"
                ] = None
    ...
    def _add_current_next_dates(
        ...
        """
        Given week information on when they open, add the current and next
        opening dates.
        """
        ...
        week_info_dataframe["Time diff Req Date"] = (
            pd.to_datetime(requested_date).date()
            - week_info_dataframe["Date_of_Last_SO"]
        ).apply(lambda x: x.days) % 14
        week_info_dataframe.loc[
            week_info_dataframe["Time diff Req Date"] == 0, "Time diff Req Date"
        ] = 14
        week_info_dataframe.loc[
            (week_info_dataframe["Time diff Req Date"] == 0)
        ...
        week_info_dataframe.loc[
            ...
            pd.Timestamp(requested_date) + pd.to_timedelta(
                week_info_dataframe["Time diff Req Date"], unit='D'
            )
        ).dt.date
    ...
    def _filter_time_information(
        ...
        """
        Given the dataframe containing the date information, filter the
        dataframe based on the time information.
        """
        ...
        for col in ["Starting Time", "Ending Time", "Max Starting Time", "Max Ending Time"]:
            date_info_dataframe[col] = pd.to_datetime(
                date_info_dataframe[col], format="%H:%M:%S"
            ).dt.time

        # if max(Starting Time, asked_time_start) > min(Ending Time, asked_time_end):
        # Time does not exist
        date_info_dataframe.loc[
            date_info_dataframe["Max Starting Time"] < date_info_dataframe["Starting Time"],
            "Max Starting Time"
        ] = date_info_dataframe["Starting Time"]
        date_info_dataframe.loc[
            date_info_dataframe["Max Ending Time"] > date_info_dataframe["Ending Time"],
            "Max Ending Time"
        ] = date_info_dataframe["Ending Time"]

        date_info_dataframe.loc[
            date_info_dataframe["Max Starting Time"] > date_info_dataframe["Max Ending Time"],
            "Open on Requested Date"
        ] = False
